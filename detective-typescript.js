 ...
 * @return {String[]}
 */
function detectiveTypeScript(src, options = {}) {
...
export const tsx = (src, options = {}) => {
  return detectiveTypeScript(src, { ...options, jsx: true });
};
...
function isTypeImports(node) {
  ...
  if (
    ...
    node.specifiers?.every((n) => n.importKind === "type")
  ) {
    return true;
  }
}
...
function isTypeExports(node) {
  ...
  if (
    node.specifiers?.length &&
    node.specifiers?.every((n) => n.exportKind === "type")
  ) {
    return true;
  }
}

export default detectiveTypeScript;
