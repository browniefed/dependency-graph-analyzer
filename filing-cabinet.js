 ...
 */
export default function filingCabinet(options = {}) {
...
export const supportedFileExtensions = Object.keys(defaultLookups);
 ...
 */
export function register(extension, lookupStrategy) {
 ...
 */
export function unregister(extension) {
  delete defaultLookups[extension];
  supportedFileExtensions = Object.keys(defaultLookups);
}
...
function tsLookup({
  ...
  if (namedModule.resolvedModule) {
    ...
    if (namedModule.resolvedModule.extension === ".d.ts" && noTypeDefinitions) {
      const resolvedFileNameWithoutExtension = result.replace(
        namedModule.resolvedModule.extension,
        ""
      );
      try {
        result = ts.resolveJSModule(
          resolvedFileNameWithoutExtension,
    ...
    result =
      lookUpLocations.find((location) => ts.sys.fileExists(location)) || "";
  }
