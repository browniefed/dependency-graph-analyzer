 ...
 * @return {Object}
 */
export default function dependencyTree(options = {}) {
 ...
 */
export const toList = function (options = {}) {
 ...
 * @return {Array}
 */
export const _getDependencies = function (config = {}) {
...
function traverse(config = {}) {
  ...
  let dependencies = _getDependencies(config);
...
function getDirectory(localConfig) {
  ...
  return (
    getProjectPath(path.dirname(localConfig.filename)) || localConfig.directory
  );
}
...
function getProjectPath(filename) {
  try {
    ...
    const packageSubPathPath = nodeModuleParts
      .pop()
      .split(path.sep)
      .filter(Boolean);
    const packageName = packageSubPathPath[0].startsWith("@")
      ? `${packageSubPathPath[0]}${path.sep}${packageSubPathPath[1]}`
      : packageSubPathPath[0];

    return path.normalize(
      [...nodeModuleParts, `${path.sep}${packageName}`].join("node_modules")
    );
  } catch {
    debug(
      `Could not determine the root directory of package file ${filename}. Using default`
    );
    return null;
  }
}
